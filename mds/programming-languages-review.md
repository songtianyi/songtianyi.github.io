...
* 为编程语言的选型提供依据
* 系统地了解各种类型特性，语法特性和编程范式

编程语言往大了讲是很复杂的，语法，类型系统，编译原理，编译器，解释器，内存模型，并发模型，工具链等等，单拿一点出来都能写一本书。所以本文要介绍的内容限定在类型系统(type systems)，语法(syntax)和编程范式(programming paradigms)上。
