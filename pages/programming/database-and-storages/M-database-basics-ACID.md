...
在 Read Commited 的基础上，对正在读取的行加读锁，不允许修改，直到事物结束。如果读锁是行级别的，可能会出现，同一个事物中，一开始读到的行数和后来读到的行数不一致。
...
undo log 主要记录事物结束之前的操作，当事物失败进行回滚，这和 redo log 是相反的，redo log 或者说 WAL 是记录事物成功后的整个事物操作记录。
